...
void CreateMatrix(void) {
    ...
    for (a = 0; a < nCol; a++) {
        ...
        for (b = 0; b < nRow; b++) {
            if (Data[a][b] != 0) {
                ...
                #pragma omp critical
                Matrix[a][b].Left = &Matrix[i][j] - Matrix;
                // Right pointer
                i = a;
                j = b;
                do {
                ...
                #pragma omp critical
                Matrix[a][b].Right = &Matrix[i][j] - Matrix;
                // Up pointer
                i = a;
                j = b;
                do {
                ...
                #pragma omp critical
                Matrix[a][b].Up = &Matrix[i][j] - Matrix;
                // Down pointer
                i = a;
                j = b;
                do {
                ...
                #pragma omp critical
                Matrix[a][b].Down = &Matrix[i][j] - Matrix;
                // Header pointer
                #pragma omp critical
                Matrix[a][b].Header = &Matrix[a][nRow - 1] - Matrix;
                #pragma omp critical
                Matrix[a][b].IDNum = b;
                // Row Header
                #pragma omp critical
                RowHeader[b] = &Matrix[a][b] - Matrix;
            }
        }
    }
    ...
    // Insert root
    Root.IDName = 'R';
    Root.Left = &Matrix[nCol - 1][nRow - 1] - Matrix;
    Root.Right = &Matrix[0][nRow - 1] - Matrix;
    Matrix[nCol - 1][nRow - 1].Right = &Root - Matrix;
    Matrix[0][nRow - 1].Left = &Root - Matrix;
}
...
int countOnes(struct str_node *c) {
    struct str_node *r;
    int i = 0;
    for (r = c->Down + Matrix; r != c; r = r->Down + Matrix){
        i++;
    }
    return i;
}
...
struct str_node *ChooseColumn(void) {
    ...
    for (c = RootNode->Right + Matrix; c != RootNode; c = c->Right + Matrix) {
        if (minOnes > c->count){
            minOnes = c->count;
            best = c;
        }
...
void Cover(struct str_node *ColNode) {
    ...
    (ColNode->Right + Matrix)->Left = ColNode->Left;
    (ColNode->Left+ Matrix)->Right = ColNode->Right;
    for (RowNode = ColNode->Down + Matrix; RowNode != ColNode; RowNode = RowNode->Down+ Matrix) {
        for (RightNode = RowNode->Right + Matrix; RightNode != RowNode;
             RightNode = RightNode->Right + Matrix) {
            (RightNode->Up + Matrix)->Down = RightNode->Down;
            (RightNode->Down + Matrix)->Up = RightNode->Up;
            (RightNode->Header+ Matrix)->count--;
        }
    }
}
...
void UnCover(struct str_node *ColNode) {
    ...
    for (RowNode = ColNode->Up + Matrix; RowNode != ColNode; RowNode = RowNode->Up+ Matrix) {
        for (LeftNode = RowNode->Left + Matrix; LeftNode != RowNode;
             LeftNode = LeftNode->Left + Matrix) {
            (LeftNode->Up + Matrix)->Down = LeftNode - Matrix;
            (LeftNode->Down + Matrix)->Up = LeftNode - Matrix;
            (LeftNode->Header + Matrix)->count++;
        }
    }
    (ColNode->Right+ Matrix)->Left = ColNode - Matrix;
    (ColNode->Left + Matrix)->Right = ColNode - Matrix;
}

void SolutionRow(struct str_node *RowNode) {
    Cover(RowNode->Header + Matrix);
    struct str_node *RightNode;
    for (RightNode = RowNode->Right + Matrix; RightNode != RowNode;
         RightNode = RightNode->Right + Matrix) {
        Cover(RightNode->Header + Matrix);
    }
}
...
void Search(int k) {
    ...
    if ((RootNode->Left + Matrix == RootNode &&
         RootNode->Right + Matrix == RootNode) ||
        k == (Size * Size - MaxK)) {
        // Valid solution!
    ...
    struct str_node *RightNode;
    for (RowNode = Column->Down + Matrix; RowNode != Column && !Finished;
         RowNode = RowNode->Down + Matrix) {
        ...
        for (RightNode = RowNode->Left + Matrix; RightNode != RowNode;
             RightNode = RightNode->Left + Matrix) {
            UnCover(RightNode->Header + Matrix);
        }
        Result[--nResult] = 0;
    }
    #pragma omp taskwait
...
int main(void) {
    ...
    for (r = RootNode->Right + Matrix; r != RootNode; r = r->Right + Matrix) {
        r->count = countOnes(r);
    }
