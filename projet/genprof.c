...
void permute(int *, int *, int, int,
             int); // Permute les lignes et colonnes d'une matrice
void injecte(int *, int *, int, int,
             int);      // Insère une matrice  nXn dans une matrice n^2 X n^2
void print(int *, int); // Affiche une matrice n X n

// Constrution d'un Sudoku
// Le Sudoku M est vu comme une grille nXn constituée de blocs S de dimention
// nXn Chaque bloc contient des entiers entre 0 et n^2 sans répétition des
// entiers >0 L'entier 0 représente une case vide Chaque bloc S est construit en
// permutant adéquatement les lignes et colonnes d'un bloc T fixe généré
// aléatoirement.
int main(int argc, char const *argv[]) {
    ...
    // par 0 dans une proportion de 1/2
    for (i = 0; i < n * n; i++)
        for (j = 0; j < n * n; j++)
            if (rand() % 2)
                M[i * n * n + j] = 0;
...
// Cette fonction permute les lignes et colonnes de T et place le résultat dans
// S. T demeure inchangé
