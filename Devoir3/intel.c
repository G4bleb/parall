...
int main(int argc, char **argv){
    ...
    if (id != 0)
        MPI_Get(&localbuffer[0], NUM_ELEMENT-2, MPI_INT, id - 1, 0+2, NUM_ELEMENT,
                MPI_INT, win);
    else
        MPI_Get(&localbuffer[0], NUM_ELEMENT-2, MPI_INT, num_procs - 1, 0+2,
    ...
    if (id < num_procs - 1)
        MPI_Put(&localbuffer[0], NUM_ELEMENT, MPI_INT, id + 1, 0, NUM_ELEMENT,
                MPI_INT, win);
    else
